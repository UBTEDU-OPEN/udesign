...
export const Block = ({ title, description, background = true, transform, compact, debug, todo, children, className }: DemoBlockProps) => {
  ...
  const code = reactElementToJSXString(React.isValidElement(children) ? children : <>{children}</>, { maxInlineAttributesLineLength: 80, showFunctions: true, sortProps: false, useBooleanShorthandSyntax: false });
