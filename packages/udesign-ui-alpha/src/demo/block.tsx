...
import { getEnv, NativeProps } from '../utils';
...
export const Block = ({ title, description, background, transform, compact, debug, todo, children, className }: DemoBlockProps) => {
  const cls = classNames(background ? 'bg-gray-100' : '', transform ? 'translate-x-0 translate-y-0' : '', compact ? '' : 'p-6', className);
  ...
  return debug && getEnv() === 'production' ? null : (
    <>
      {title || description ? (
        <div className='py-4'>
      ...
      {debug ? <Tag type='warning'>Dev Only</Tag> : null}
